							...
							if (is_in_view($(this))){
								...
								if ($urlVideo !== undefined) {
									...
									if ($urlVideo.indexOf('youtube') !== -1) {
										...
										var videoCode = $urlVideo.substring($urlVideo.length - 11, $urlVideo.length);
										$("body").append('<script>var tag=document.createElement("script");tag.src="https://www.youtube.com/iframe_api";var player,firstScriptTag=document.getElementsByTagName("script")[0];function onYouTubeIframeAPIReady(){player=new YT.Player("player",{width:"100%",videoId:"'+videoCode+'",playerVars:{autoplay:1,playsinline:1,loop:1,rel:0,showinfo:0},events:{onReady:onPlayerReady,onStateChange:onPlayerStateChange}})}function onPlayerReady(e){e.target.mute(),e.target.playVideo()}function onPlayerStateChange(e){e.data==YT.PlayerState.ENDED&&(player.seekTo(0),player.playVideo())}function stopVideo(){player.stopVideo()}firstScriptTag.parentNode.insertBefore(tag,firstScriptTag);</script>');
										...
										$(this).html('<iframe allow="autoplay; encrypted-media" title="'+titleVideo+'" src="' + srcVideo + '?autoplay=1&amp;muted=1&loop=1" frameborder="0" allowfullscreen></iframe>');
									} else {
										$(this).html('<video src="'+$urlVideo+'" title="'+titleVideo+'" preload="none" loop muted playsinline>');
									}
								}
